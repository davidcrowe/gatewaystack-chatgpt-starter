...
import { looksLikeJsonRpc, handleMcp } from "../handlers/mcpHandler-firebase";
import {
  verifyBearer,
...
import { getFirebaseIdTokenForUid } from "../handlers/firebaseAdapter";
import {
  OAUTH_ISSUER,
  OAUTH_AUDIENCE,
  OAUTH_SCOPES,
...
// Re-export for other helpers (like authHelpers/wellKnown) if desired
...
async function baseToolGatewayImplv3(req: Request, res: Response): Promise<void> {
  ...
  if ((req.method === "GET" || req.method === "HEAD") &&
    ...
    try {
      ...
      res
        .status(502)
        .json({ error: "discovery_fetch_failed", detail: e?.message || String(e) });
    }
    return;
  }
  ...
  try {
    ...
    const raw = await proxyablClient.callTool(name, req.body ?? {}, idToken);
    ...
    console.log("[proxyabl] rest call normalized", {
      name,
      httpOk,
      httpStatus,
  ...
  return new Promise<void>((resolve) => {
    explicablMiddleware(req as any, res as any, (expErr?: any) => {
      if (expErr) {
        ...
        if (!res.headersSent) {
          res.status(500).json({
            ok: false,
            error: {
              code: "EXPLICABL_ERROR",
              message: expErr?.message || "Explicabl middleware error",
            },
          });
        }
        return resolve();
      ...
      Promise.resolve(baseToolGatewayImplv3(req, res))
        ...
        .catch((e) => {
          console.error(
            "[toolGatewayImplv3] Unhandled error from baseToolGatewayImplv3",
            e
          );
          if (!res.headersSent) {
            res.status(500).json({
